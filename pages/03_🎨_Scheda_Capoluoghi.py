...
try:
    df__list__CTI.set_index('reg', inplace=True)
except:
    ''
try:
    df__list__COB.drop(['location'], axis=1, inplace=True) 
except:
    ''
#
#
...
#
# TEMPERATURE PLOT
df__DBT__CTI_plot = df__DBT__CTI.copy()
df__DBT__COB_plot = df__DBT__COB_capo.copy()
df__DBT__CTI_plot.columns = df__DBT__CTI_plot.columns.str.replace('DBT\|','')
df__DBT__CTI_plot = df__DBT__CTI_plot.convert_dtypes(convert_floating=True)
df__DBT__COB_plot = df__DBT__COB_plot.convert_dtypes(convert_floating=True)

########## ########## ########## ########## ########## ########## ##########
st.markdown('TEST')
st.dataframe(df__DBT__COB_plot)
########## ########## ########## ########## ########## ########## ##########

#  Set datetime index
df__DBT__CTI_plot['datetime']=pd.to_datetime(df__DBT__CTI_plot.index)
df__DBT__CTI_plot.set_index(['datetime'], inplace=True, drop=True)
#
df__DBT__COB_plot['datetime']=pd.to_datetime(df__DBT__CTI_plot.index)
df__DBT__COB_plot.set_index(['datetime'], inplace=True, drop=True)

df__DBT__CTI_plot.rename(columns=lambda x: x.split('__',1)[-1], inplace=True)
#
#
#
#
...
gdf_stations = gpd.GeoDataFrame(
    df__list__COB_capo,
    geometry=gpd.points_from_xy(df__list__COB_capo.lon, df__list__COB_capo.lat),
    crs=provinces.crs,
    )
...
filtered_df_mapped = df_mapped[df_mapped['prov_acr'].isin(df__list__ITA_capo['province'])]
#
#
#
#
...
with colA:
    ...
    selected_province = st.selectbox("Seleziona una provincia", options=sorted(df__list__ITA_capo.province), index=4)

p = selected_province
filtered_df_COB = filtered_df_mapped[filtered_df_mapped['prov_acr'].str.contains(p)]
filtered_df_CTI = df__list__CTI[df__list__CTI['province'].str.contains(p)]
...
filtered__df__DBT__CTI_plot = df__DBT__CTI_plot.loc[:, [col for col in df__DBT__CTI_plot.columns if p in col[:2]]]
wmo_code_selected = filtered_df_COB.wmo_code.to_list()

filtered__df__DBT__COB_plot = pd.DataFrame()   

for wmo in wmo_code_selected:
    df_sel = df__DBT__COB_plot.loc[:, [col for col in df__DBT__COB_plot.columns if str(wmo) in col[:6]]]
    filtered__df__DBT__COB_plot = pd.concat([filtered__df__DBT__COB_plot, df_sel], axis=1)
...
with colB:
    ...
    with tab1:
        ...
        with colB2:
            df__list__COB_capo['wmo_code'] = df__list__COB_capo['wmo_code'].astype(str)

            # Ensure selected_wmo_code is a list
            if isinstance(wmo_sel_plotting, list):
                filtered_df = df__list__COB_capo[df__list__COB_capo['wmo_code'].isin(wmo_sel_plotting)]
            else:
        ...
        if wmo_sel_plotting:
            selected_columns = [col for col in filtered__df__DBT__COB_plot.columns if any(code in col for code in wmo_sel_plotting)]
            st.write(selected_columns)

            # Display the selected columns of the DataFrame
            if selected_columns:
                double_filtered__df__DBT__COB_plot = filtered__df__DBT__COB_plot[selected_columns]
                # st.dataframe(double_filtered__df__DBT__COB_plot[:3])
            else:
        ...
        try:
            df_CTI_filtered, df_COB_filtered, df_diff, fig_weekly, fig_diff_monthly = calculate_and_plot_differences(
                ...
                df1=filtered__df__DBT__CTI_plot,
                df2=double_filtered__df__DBT__COB_plot,
        ...
        with colB2:
            ...
            fig_line_CTI_COB = generate_line_chart(
                ...
                df_data_A = filtered__df__DBT__CTI_plot,
                df_data_B = double_filtered__df__DBT__COB_plot,
    ...
    with tab2:
        ...
        # Filter CTI and COB dataframes for plotting
        df__DBT__CTI_plot.index = df__DBT__CTI_plot.index.map(lambda x: x.replace(year=sel_year))
        filtered__df__DBT__CTI_plot = df__DBT__CTI_plot.loc[:, [col for col in df__DBT__CTI_plot.columns if p in col[:2]]]
        wmo_code_selected = filtered_df_COB.wmo_code.to_list()
...
##### ##### #####
    ...
    with tab3:
        ...
        # Filter CTI and COB dataframes for plotting
        filtered__df__DBT__CTI_plot.index = filtered__df__DBT__CTI_plot.index.map(lambda x: x.replace(year=int(sel_year_tab3)))
...
frames = {'CTI' : filtered__df__DBT__CTI_plot,    'COB' : filtered__df__DBT__COB_plot,    'MSTAT' : df__MSTAT_DBT_plot,}
